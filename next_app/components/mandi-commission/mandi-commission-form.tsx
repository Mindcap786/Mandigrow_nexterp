    ...
    return (
        <div className="max-w-[1400px] mx-auto space-y-4">
            <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                    <h1 className="text-2xl font-black text-slate-900 tracking-tight leading-none">{t('sales.purchase_sale')}</h1>
                    <p className="text-[12px] font-bold text-slate-500 uppercase tracking-widest mt-1">{t('sales.single_screen')}</p>
                </div>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">{t('sales.date')}</Label>
                ...
                <div>
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">{t('sales.lot_no')}</Label>
                ...
                <div>
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">{t('sales.global_unit')}</Label>
                    <div className="mt-1">
                        <UnitCombobox 
                            value={globalUnit} 
                ...
                <div>
                    <Label className="text-[10px] font-black uppercase tracking-widest text-slate-500 ml-1">{t('sales.vehicle_no')}</Label>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-start">
                ...
                <div className="bg-slate-50/50 border border-slate-200 rounded-2xl p-4 min-h-[480px]">
                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4 flex items-center gap-2">
                        <User className="w-4 h-4 text-emerald-600" /> {t('sales.latest_arrivals')} ({rows.length})
                    </h3>
                    ...
                    {rows.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-slate-400 mt-20">
                            ...
                            <p className="text-sm font-bold">{t('sales.no_farmers')}</p>
                            <p className="text-xs">{t('sales.fill_details')}</p>
                        </div>
                    ) : (
                ...
                <div className="bg-white border-2 border-emerald-100 rounded-2xl p-5 pt-4 shadow-lg shadow-emerald-500/5 relative overflow-hidden">
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <Input ref={qtyRef} type="number" min={0} step="any" value={currentRow.qty || ""} onChange={(e) => handleCurrentRowChange("qty", parseFloat(e.target.value) || 0)} onKeyDown={(e) => handleKeyDown(e, rateRef)} className={inputCls} placeholder={t('sales.qty')} />
                            </div>
                             <div>
                                <Input ref={rateRef} type="number" min={0} step="any" value={currentRow.rate || ""} onChange={(e) => handleCurrentRowChange("rate", parseFloat(e.target.value) || 0)} onKeyDown={(e) => handleKeyDown(e, lessPercentRef)} className={inputCls} placeholder={t('sales.price')} />
                            </div>
                        </div>

                        <div className="grid grid-cols-2 gap-4">
                             <div>
                                <Input ref={lessPercentRef} type="number" min={0} step="any" value={currentRow.lessPercent || ""} onChange={(e) => handleCurrentRowChange("lessPercent", parseFloat(e.target.value) || 0, "lessPercent")} onKeyDown={(e) => handleKeyDown(e, lessUnitsRef)} className={inputCls + " text-red-600"} placeholder={t('sales.less_percent')} />
                            </div>
                            <div>
                                <Input ref={lessUnitsRef} type="number" min={0} step="any" value={currentRow.lessUnits || ""} onChange={(e) => handleCurrentRowChange("lessUnits", parseFloat(e.target.value) || 0, "lessUnits")} onKeyDown={(e) => handleKeyDown(e, loadingRef)} className={inputCls + " text-red-600"} placeholder={t('sales.less_weight')} />
                            </div>
                        </div>

                        <div className="grid grid-cols-1 gap-4">
                            <div>
                                <Input ref={loadingRef} type="number" min={0} step="any" value={currentRow.loadingCharges || ""} onChange={(e) => handleCurrentRowChange("loadingCharges", parseFloat(e.target.value) || 0)} onKeyDown={(e) => handleKeyDown(e, commPctRef)} className={inputCls} placeholder={t('sales.loading_charges')} />
                            </div>
                        </div>
                        
                        <div>
                            <Input ref={commPctRef} type="number" min={0} step="any" value={currentRow.commissionPercent || ""} onChange={(e) => handleCurrentRowChange("commissionPercent", parseFloat(e.target.value) || 0)} onKeyDown={(e) => handleKeyDown(e, addBtnRef)} className={inputCls} placeholder={t('sales.commission_percent')} />
                        </div>
                        ...
                        <div className="pt-2">
                            ...
                            >
                                [{t('sales.add')}] <Plus className="ml-2 w-6 h-6" />
                            </Button>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-4 bg-amber-50 border border-amber-200 p-4 rounded-xl shadow-sm">
                    <div className="flex items-center justify-between mb-3">
                        <div className="flex flex-col">
                            <span className="text-[10px] font-black uppercase tracking-widest text-amber-700">{t('sales.add_crates')}</span>
                            <span className="text-[9px] font-bold text-amber-600/70">{t('sales.bill_crates_separately')}</span>
                        </div>
                        <Switch
            ...
            <div className="flex justify-center mt-12 mb-20">
                ...
                >
                    {isCommitting ? (t('sales.generating_bills')) : (t('sales.submit_print'))}
