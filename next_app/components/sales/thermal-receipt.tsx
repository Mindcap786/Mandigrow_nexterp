...
export default function ThermalReceipt({ sale, organization, lang, itemTranslations, partyTranslation }: ThermalReceiptProps) {
    ...
    return (
        <div className="bg-white text-black font-mono text-[15px] leading-tight w-full mx-auto font-bold pb-8" style={{ filter: 'grayscale(100%) contrast(1000%)', maxWidth: '400px' }}>
            <div className="text-center mb-4">
                <h1 className="font-black text-xl leading-tight uppercase">{orgName}</h1>
                <p className="text-[13px] uppercase mt-1 tracking-widest">{t('TAX_INVOICE_BILL' as any, activeLang)}</p>
            </div>
            
            <div className="mb-4 border-b-2 border-dashed border-black pb-2 text-[14px]">
                ...
                {(sale.contact?.full_name || sale.contact?.name || sale.buyer_name) && (
                    <div className="flex justify-between mt-1 pt-1 border-t-2 border-dotted border-black">
                        ...
                        <span className="font-black uppercase text-right leading-tight max-w-[180px]">
            ...
            <table className="w-full text-left mb-4 text-[14px]">
                <thead>
                    <tr className="border-b-2 border-dashed border-black">
                        <th className="pb-1 font-black w-1/2">{t('ITEM' as any, activeLang)}</th>
                        <th className="pb-1 font-black text-right w-[15%]">{t('QTY' as any, activeLang)}</th>
                        <th className="pb-1 font-black text-right w-[15%]">{t('RATE' as any, activeLang)}</th>
                        <th className="pb-1 font-black text-right w-[20%]">{t('AMT' as any, activeLang)}</th>
                    </tr>
                </thead>
                <tbody>
                    {items.map((c: any, i: number) => {
                        ...
                        return (
                            <tr key={i}>
                                <td className="py-2 pr-1 font-black">
            ...
            <div className="border-t-2 border-dashed border-black pt-2 mb-4 space-y-1 text-[15px]">
            ...
            {/* Signature Area */}
            <div className="mt-8 pt-4 border-t-2 border-black text-center text-[13px] uppercase font-black tracking-widest">
                {t('THANK_YOU' as any, activeLang)}
            </div>
            {organization?.phone && (
                <div className="text-center text-[11px] font-bold mt-2 italic">
                    {organization.phone}
                </div>
            )}
        </div>
