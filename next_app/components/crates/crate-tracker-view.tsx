...
export function CrateTrackerView() {
    ...
    const issueGroups = Array.from(new Set(issues.map(r => r.issue_id))).map(id => {
        ...
        return { id, party_name: first?.party_name, party_type: first?.party_type, issue_date: first?.issue_date, expected_return_date: first?.expected_return_date, status: first?.status, rows, totalBalance, totalValue, hasOverdue, party_id: first?.party_id }
    }).filter(g => {
