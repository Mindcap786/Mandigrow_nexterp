...
export default function StockPage() {
    ...
    const aggregatedStock = filteredStock.reduce((acc, curr) => {
        ...
        if (!acc[curr.item_id]) {
            ...
            if (curr.secondary_equivalent > 0 && u.toUpperCase() !== curr.secondary_uom?.toUpperCase()) {
                acc[curr.item_id].units[curr.secondary_uom.toUpperCase()] = parseFloat(curr.secondary_equivalent)
            }
        } else {
            acc[curr.item_id].units[u] = (acc[curr.item_id].units[u] || 0) + parseFloat(curr.current_stock)
            if (curr.secondary_equivalent > 0 && u.toUpperCase() !== curr.secondary_uom?.toUpperCase()) {
                acc[curr.item_id].units[curr.secondary_uom.toUpperCase()] = (acc[curr.item_id].units[curr.secondary_uom.toUpperCase()] || 0) + parseFloat(curr.secondary_equivalent)
            }
