...
def get_sale_master_data(org_id: str = None) -> dict:
    ...
    buyers_raw = frappe.db.sql(f"""
        SELECT name AS id, full_name AS display_name, contact_type, city, internal_id
        FROM `tabMandi Contact`
        WHERE {' AND '.join(b_where)}
        ORDER BY full_name ASC
        LIMIT 500
    """, b_params, as_dict=True)
    buyers = [{"id": b.get("id") or "", "name": b.get("display_name") or b.get("id") or "Unknown", "type": b.get("contact_type") or "", "city": b.get("city") or "", "internal_id": b.get("internal_id") or ""} for b in buyers_raw]
...
def record_quick_purchase(**kwargs) -> dict:
    ...
    for l in p_lots:
        items.append({
            ...
            "loading_label": l.get("loading_label"),
    ...
    try:
        ...
        for idx, item in enumerate(items):
            ...
            lot_data = {
                ...
                "loading_label": item.get("loading_label"),
