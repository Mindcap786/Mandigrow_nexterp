...
async function testRPC() {
    // get a contact and org known to have lots
    const { data: entries } = await supabase.from('ledger_entries').select('organization_id, contact_id').like('description', '%LOT-%').limit(1).single();
    if (!entries) return console.log("No contacts with lots found.");
    ...
    if (error) {
        ...
        const withLots = data.transactions.filter(t => t.description && t.description.includes('LOT-'));
        console.log(JSON.stringify(withLots.slice(0, 3), null, 2));
    }
}
